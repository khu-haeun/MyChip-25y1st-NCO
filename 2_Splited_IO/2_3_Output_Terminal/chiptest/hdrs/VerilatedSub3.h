...
//2. component
    ...
    sc_signal<unsigned int>  sOutData;
...
//4. con/destructor
        ...
        outData ("outData")
    {
        ...
        sensitive << clk
                  << inSelXY << inMode 
                  << inRdy << inIsNeg << inX << inY << outData;
        
        // Instantiate Verilated PE
        vSub3 = new Voutput_terminal("vSub3");
        vSub3->clk     (clk);
        vSub3->selXY   (sInSelXY);
        vSub3->selSign (sInMode);
        vSub3->Rdy   (sInRdy);
        vSub3->ISin (sInIsNeg);
        vSub3->Xin     (sInX);
        vSub3->Yin     (sInY);
