...
#include "VerilatedNCOBoard.h"
#include "WrapperOfNCOBoard.h"
...
typedef struct MemberOfQPSK{
	...
	//HW
#ifdef DEV
	VerilatedNCOBoard*		NCO; //init when constructed
#else
	WrapperOfNCOBoard*		NCO;
#endif

	uint32_t				inFCW; //init when constructed
	const int bitSerialLatency = 7;
	//WrapperOfNCOBoard*		mNCO;
...
//1. HW signal
	...
	//output mpw
	sc_signal<sc_uint<12>>	tbOutX;
 	sc_signal<sc_uint<12>>	tbOutY;
	sc_signal<bool>			tbOutVldX;
	sc_signal<bool>			tbOutVldY;
...
//4. con/destructor
        ...
        tbOutY("tbOutY")
	{
		...
		mQPSK.NCO = new VerilatedNCOBoard("HW_NCO");
		//mQPSK.NCO = new WrapperOfNCOBoard("mpwNCO");
		mQPSK.NCO->clk(clk);
		mQPSK.NCO->en(tbEn);
		mQPSK.NCO->inMode(tbInMode);
		mQPSK.NCO->inFCW(tbInFCW);
		mQPSK.NCO->outX(tbOutX);
		mQPSK.NCO->outY(tbOutY);
		mQPSK.NCO->outVldX(tbOutVldX);
		mQPSK.NCO->outVldY(tbOutVldY);
