...
#define N_RX    4
    ...
    void wrapperThread(void)
    {
        ...
        while(true)
        {
			...
			outVldX.write((rxPacket[0] & 0x80) ? 1:0);   //1000_0000
            oX = (uint16_t)(rxPacket[0] & 0x0F) << 8; //MSB 4bit
            oX = oX | rxPacket[1];                  //LSB 8bit
            outX.write(oX);

            outVldY.write((rxPacket[2] & 0x80) ? 1:0);   //1000_0000
            oY = (uint16_t)(rxPacket[2] & 0x0F) << 8; //MSB 4bit
            oY = oY | rxPacket[3];                  //LSB 8bit
            outY.write(oY);
